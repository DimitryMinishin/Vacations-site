...
socketServer.sockets.on("connection", (socket) => {
  try {
    ...
    console.log(
      `${userId} has been connected, Total clients: ${userIdToSocketsMap.size}`
    );

    socket.on("update-vacation", async (vacation) => {
      socketServer.emit("get-vacations", await vacationsDal.getAllVacations());
    });
...
function disconnect(userId) {
  ...
  console.log(
    `${userId} client has been disconnected. Total clients: ${userIdToSocketsMap.size}`
  );
}
